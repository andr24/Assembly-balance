...
import { Plus, Trash2, Settings2, Download, Upload, BarChart3, LayoutDashboard, Zap, Info, ChevronDown, FileJson, FileSpreadsheet } from 'lucide-react';
      ...
      <div className="flex items-center gap-8">
        ...
        <div className="flex items-center gap-3">
          <select 
            ...
            className="bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500"
          >
            {lines.map(l => (
          ...
          <button 
            onClick={onAddLine}
            className="bg-white text-slate-600 hover:bg-slate-50 px-3 py-2 rounded-lg font-medium transition-all border border-slate-200 shadow-sm"
          >
            <Plus size={18} />
          </button>
        </div>
        ...
        <div className="relative">
          ...
          {showSettingsMenu && (
            <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-slate-200 rounded-xl shadow-xl z-50 p-2 space-y-1">
              <button 
                onClick={() => setSettings({ ...settings, autoBalanceAll: !settings.autoBalanceAll })}
                className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-bold ${settings.autoBalanceAll ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`}
              >
                Auto-Balance
                <Zap size={16} fill={settings.autoBalanceAll ? "currentColor" : "none"} />
              </button>
              <button 
              ...
              <div className="border-t border-slate-100 my-1 pt-1">
                <div className="px-3 py-2 text-xs font-bold text-slate-400 uppercase">Constrained Balance</div>
                <div className="flex items-center gap-2 px-3 pb-2">
                  <input 
                    type="number" 
                    placeholder="Pool"
                    value={settings.totalWorkersPool || ''}
                    onChange={e => setSettings({ ...settings, totalWorkersPool: Number(e.target.value) })}
                    className="w-16 bg-slate-50 border border-slate-200 rounded px-2 py-1 text-xs font-bold outline-none focus:ring-1 focus:ring-purple-500"
                  />
                  <button 
                    onClick={() => setSettings({ ...settings, useConstrainedBalance: !settings.useConstrainedBalance })}
                  ...
                  >
                    {settings.useConstrainedBalance ? "ON" : "OFF"}
                  </button>
                </div>
              </div>
            </div>
